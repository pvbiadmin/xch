 ...
 * @since version
 */
function view_admin_efund_request_logs(): string
{
		...
		foreach ($result as $log)
		{
			...
			$str .= '<tr>
				...
				'uid=' . $user->id . '">' . $user->username . '</a></td>
                    ...
                    <td>' . number_format($log->request_total, 8) . ' ' . $efund_name . '</td>
                </tr>';
		}
 ...
 * @since version
 */
function view_user_efund_request_logs($user_id): string
{
...
function view_logs($user_id, $usertype, $admintype): string
{
	...
	{
		$str .= view_admin_efund_request_logs();
	}
	else
	{
		$str .= view_user_efund_request_logs($user_id);
	}
