...
namespace BPL\Jumi\Request_Share_Fund_Pending;
...
function main()
{
	...
	$str .= '<h1>Pending ' . settings('ancillaries')->share_fund_name . ' Requests</h1>';
...
//	$currency = settings('ancillaries')->currency;
	...
	$str .= '<tr>
		...
		$user_request->username . '</a>' . '</td>
			<td>' . number_format($user_request->share_fund, 2) . ' ' . /*$share_fund_name .*/
		'</td>
			<td>' . number_format($user_request->amount, 2) . ' ' . /*$share_fund_name .*/
		'</td>
...
//			$str .= '<td>' . number_format($request->amount, 2) . '</td>';
			...
			$str .= '<p>Upon Payment confirmation, Admin can now transfer <b>' .
				...
				$share_fund_name . ' to <b>' . $user->username . '</b></p>';
			...
			$str .= '<li>';
			$str .= '<a href="' . sef(138) . qs() . 'uid=' . $request->request_id . '&mode=1">Approve</a>';
			$str .= '</li>';
			$str .= '<li>';
			$str .= '<a href="' . sef(138) . qs() . 'uid=' . $request->request_id . '&mode=2">Deny</a>';
			$str .= '</li>';
			$str .= '</ul>';
			$str .= '</div>';
			$str .= '</div>';
	...
	{
		$str .= '<hr><p>No pending ' . $share_fund_name . ' requests.</p>';
	}
 ...
 * @since version
 */
function update_user_share_fund($amount, $user_id)
{
	$db = db();

	update(
		'network_users',
		['share_fund = share_fund + ' . $amount],
		['id = ' . $db->quote($user_id)]
	);
}
 ...
 * @since version
 */
function update_share_fund_request($uid)
{
	$db = db();
...
function process_approve($uid)
{
	...
	$share_fund_name = $settings_ancillaries->share_fund_name;
	...
	try
	{
		...
		update_user_share_fund($user_request->amount, $user_request->id);
		update_share_fund_request($uid);

		logs_approve($uid);

		send_mail($message, $share_fund_name . ' Request Confirmed', [$user_request->email]);
...
//	send_mail($uid);
	...
	application()->redirect(Uri::root(true) . '/' . sef(138),
		$share_fund_name . ' request confirmed!', 'success');
}
...
function logs_activity_approve($uid)
{
	...
	$activity = '<b>' . $share_fund_name . ' Request Confirmed: ' . '</b><a href="' .
		sef(44) . qs() . 'uid=' . $user_request->id . '">' . $user_request->username . '</a> has requested ' .
		number_format($user_request->amount, 2) . ' ' . $share_fund_name;
...
function logs_transactions_approve($uid)
{
	...
	$details = '<b>' . $share_fund_name . ' Request Confirmed: ' . '</b><a href="' . sef(44) . qs() .
	...
	insert(
		...
		[
			$db->quote($user_request->id),
			$db->quote($share_fund_name . ' Request Confirmed'),
			$db->quote($details),
...
function logs_requests_approve($uid, $transaction_id)
{
	...
	$fund_request = share_fund_requests();

	$db = db();

	insert(
		'network_share_fund_requests',
		[
			'transaction_id',
			'amount',
			'price',
...
function process_deny($uid)
{
	...
	application()->redirect(Uri::root(true) . '/' .
		sef(138), settings('ancillaries')->share_fund_name . ' Request denied!', 'error');
}
...
function logs_deny($uid)
{
	...
	$activity = '<b>' . $share_fund_name . ' Request Denied: ' . '</b><a href="' .
...
function user_request($uid)
{
	...
	return $db->setQuery(
		...
		'INNER JOIN network_share_fund_request r ' .
		'ON u.id = r.user_id ' .
 ...
 * @since version
 */
function share_fund_requests()
{
	$db = db();

	return $db->setQuery(
		'SELECT * ' .
		'FROM network_share_fund_requests ' .
