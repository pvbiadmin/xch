...
namespace BPL\Jumi\Loans_Transfer;

require_once 'bpl/mods/ajax.php';
require_once 'bpl/mods/loans_transfer_history.php';
...
use function BPL\Mods\Loans_Transfer_History\view_row_transfers;
...
function view_form($user_id): string
{
	...
	$str = '<h1>' . $loan_fund_name . ' Transfer</h1>';
	...
	$str .= '<form method="post" onsubmit="submit.disabled = true; return true;">
        ...
        <table class="category table table-striped table-bordered table-hover">
            <tr>
                <td colspan="2"><strong>' . $loan_fund_name . ': </strong>' .
		number_format($transfer_from->loans, 5) . '</td>
            </tr>
            <tr>
...
function validate_input($user_id, $amount, $username)
{
		...
		$username === $transfer_from->username)
	{
		application()->redirect(Uri::root(true) .
			'/' . sef(139), 'Invalid Transaction!', 'error');
	}

	if ($transfer_from->loans < ($amount + $processing_fee))
	{
		application()->redirect(Uri::root(true) .
			'/' . sef(139), 'Please maintain at least ' .
	...
	if ($edit)
	{
		...
		if ($date === '')
		{
			application()->redirect(Uri::root(true) . '/' . sef(139),
...
function log_activity($user_id, $username, $amount, $date)
{
	...
	$activity = '<b>' . $loan_fund_name . ' Transfer: </b> <a href="' . sef(44) . qs() . 'uid=' . $user_id . '">' .
		$transfer_from->username . '</a> transferred ' . number_format($amount, 2) . ' ' . $loan_fund_name .
...
function log_transactions($user_id, $username, $amount, $date, &$insert_id)
{
	...
	$details = '<b>' . $loan_fund_name . ' Transfer: </b> <a href="' . sef(44) . qs() . 'uid=' . $user_id . '">' .
		$transfer_from->username . '</a> transferred ' . number_format($amount, 2) . ' ' . $loan_fund_name .
		' to <a href="' . sef(44) . qs() . 'uid=' . $transfer_to->id . '">' . $transfer_to->username . '</a>.';

	$balance = $transfer_from->loans - ((double) $amount + (double) $sa->processing_fee);
	...
	insert(
		...
		[
			$db->quote($user_id),
			$db->quote($loan_fund_name . ' Transfer'),
			$db->quote($details),
			$db->quote($amount),
			$db->quote($balance),
			$db->quote($date)
...
function process_form($user_id, $amount, $username, $date)
{
	...
	try
	{
		...
		send_mail($message, $loan_fund_name . ' Transferred Successfully!',
	...
	application()->redirect(Uri::root(true) . '/' . sef(139), number_format($amount, 2) .
		' ' . $loan_fund_name . ' transferred to ' . $username . '.', 'success');
}
...
function view_transfer_history($user_id): string
{
	...
	$str = '<h2>' . $loan_fund_name . ' Transfer History</h2>';
