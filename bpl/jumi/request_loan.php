...
namespace BPL\Jumi\Request_Loan;
...
//	}
	...
	if ($user->account_type !== 'starter')
	{
		...
		{
			$app->redirect(Uri::root(true) . '/' . sef(140) . qs() . 'uid=' . $user_id,
...
function process_delete_request($cid)
{
	...
	application()->redirect(Uri::root(true) . '/' . sef(140),
...
function validate_input($user_id, $amount, $method)
{
	...
	if ($method === 'none')
	{
		$app->redirect(Uri::root(true) . '/' . sef(140),
			'Please Select Your Payment Method!', 'error');
	}
	else
	{
...
function process_request($user_id, $amount, $method)
{
	...
	try
	{
		...
		send_mail($message, 'Loan Request', [$user->email]);
	...
	$app->redirect(Uri::root(true) . '/' . sef(140),
		'Your transaction will be posted as a pending request to be approved by admin. ', 'success');
}
...
function view_form($user_id): string
{
	...
	$str = ' <h1>Request ' . $loan_fund_name . /*'<span style="float: right">
	...
	$str .= '<p>Enter requested amount in the box, then select your preferred currency payment method, ' .
		'(minimum request is ' . number_format($min_request, 2) . ' ' . $loan_fund_name . ' up to ' .
		number_format($max_request, 2) . ' ' . $loan_fund_name . ') then press the submit button. ' .
	...
	$str .= '<td><input type="text" name="amount" placeholder="Amount (' .
		$loan_fund_name . ')" id="amount" style = "float:left">';
...
function view_pending_requests(): string
{
	...
	$str = '<h2>Pending ' . $loan_fund_name . ' Requests</h2>';

	if (empty($pending))
	{
		$str .= '<hr><p>No pending ' . $loan_fund_name . ' requests yet.</p>';
	}
	else
	{
		...
		foreach ($pending as $tmp)
		{
			...
			if (!in_array($tmp->method, ['bank', 'gcash']))
			{
				...
				$str .= '<p style="color: red; border: red">After successful transaction please screenshot the said transaction and send it to the email written below. in just eight hours this transaction is processed, and you can view your ' . $loan_fund_name . ' in your dashboard wallet, and you can choose the desired entry to join the Rewards System. any concern you can email us anytime.<br> Congrats on joining the team!<br><br> -- "Support Team".</p>';
				$str .= $contact;
				...
				$str .= '<p>Please pay <b>' . number_format($tmp->price, 2) . '</b> ' . strtoupper($currency) .
					' to the following Wallet Address:</p>
			...
			$str .= '<div class="uk-modal-footer" style="text-align: right">
						...
						<a href="' . sef(140) . qs() . 'cid=' . $tmp->request_id . '" 
