...
namespace BPL\Jumi\Request_Loan_Log;
...
//	$efund_name = /*settings('ancillaries')->efund_name*/'Savings';
	...
	{
		$str .= '<hr><p>No ' . settings('ancillaries')->loan_fund_name . ' requests yet.</p>';
	}
...
function view_user_request_logs($user_id): string
{
	...
	if ($result)
	{
		$str .= '<h1>' . $loan_fund_name . ' Activity</h1>
        ...
        <p><strong>Total ' . $loan_fund_name . ' Request: </strong>' . number_format($total, 2)/* . ' ' . $efund_name*/;
	}
	else
	{
		$str .= '<hr><p>No ' . $loan_fund_name . ' requests yet.</p>';
	}
...
function view_logs($user_id, $usertype, $admintype): string
{
	...
	$str .= '<h1>' . settings('ancillaries')->loan_fund_name . ' Request Logs</h1>';
