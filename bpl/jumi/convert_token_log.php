...
namespace BPL\Jumi\Convert_Token_Log;
 ...
 * @since version
 */
function token_convert_transactions()
{
	return db()->setQuery(
		'SELECT * ' .
		'FROM network_token_conversions c ' .
 ...
 * @since version
 */
function view_admin_token_convert_logs(): string
{
	$results = token_convert_transactions();
	...
	if (!empty($results)) {
		...
		foreach ($results as $log) {
			...
			$currency = /* in_array($log->method, ['bank', 'gcash']) ? 'PHP' : $log->method */ 'B2P';
 ...
 * @since version
 */
function user_token_convert_transactions($user_id)
{
	$db = db();
 ...
 * @since version
 */
function view_user_token_convert_logs($user_id): string
{
	...
	$results = user_token_convert_transactions($user_id);
...
function view_logs($user_id, $usertype, $admintype): string
{
	...
	$str .= '<h1>' . /* settings('ancillaries')->efund_name */ 'B2P' . ' Withdrawal Logs</h1>';

	if ($usertype === 'Admin' && $admintype === 'Super') {
		$str .= view_admin_token_convert_logs();
	} else {
		$str .= view_user_token_convert_logs($user_id);
	}
