...
namespace BPL\Ajax\Mods\Matrix\Genealogy\Regular;
...
function main($id_user, $plan)
{
	...
	details_matrix_regular(user($id_user), $plan);
...
function make_json($parent, $plan)
{
	...
	if (!empty($children))
	{
		foreach ($children as $child)
		{
			...
			details_matrix_regular($child, $plan);
 ...
 * @since version
 */
function details_matrix_regular($child, $plan): void
{
	echo '"id": "' . $child->id . '", ';
...
{
	$matrix_id = user_matrix_regular($user_id)->id;

	return fetch_all(
		'SELECT * ' .
		'FROM network_matrix_regular a ' .
 ...
 * @since version
 */
function user_matrix_regular($user_id)
{
	return fetch(
		'SELECT * ' .
		'FROM network_matrix_regular ' .
