...
namespace BPL\Ajax\Mods\Token\Trade\Table\Sell;
...
function main($user_id, $start_row)
{
	...
	$list_tot     = trade_sell($user_id);
	$list_tot_lim = trade_sell_limit($limit_from, $rows, $user_id);
	...
	if (!empty($list_tot_lim))
	{
		...
		$str .= '<h3 class="uk-panel-title" style="padding: 7px 0 0 7px">Sell</h3>';
		$str .= paginate_sell($list_tot, $rows, $start_row);
		$str = head_frame($str);
		...
		foreach ($list_tot_lim as $result)
		{
			...
			{
				$delete = ' onclick="delete_sell(' . $result->id . ')"';
 ...
 * @since version
 */
function paginate_sell($total, $rows, $start_row): string
{
	$count_total = count($total);

	$oldest_page = ($count_total - $count_total % $rows) / $rows;

	$pg_oldest = ' onclick="paginate_sell(' . $oldest_page . ')"';
	$pg_prev   = ' onclick="paginate_sell(' . ($start_row + 1) . ')"';
	$pg_next   = ' onclick="paginate_sell(' . ($start_row - 1) . ')"';
	$pg_latest = ' onclick="paginate_sell(0)"';
 ...
 * @since version
 */
function trade_sell_limit($limit_from, $limit_to, $user_id)
{
	return fetch_all(
		'SELECT * ' .
		'FROM network_fmc_trade ' .
