...
namespace BPL\Ajax\Mods\Token\Vault\Exchange;
...
function main($amount, $total, $charge, $user_id)
{
	...
	if (empty($total))
	{
		$return['error_xch'] = 'Transaction Could Not Proceed!';
	...
	{
		$return['error_xch'] = 'Enter valid amount!';
	...
	{
		$return['error_xch'] = 'Enter at least ' .
	...
	{
		$return['error_xch'] = 'Maintain at least ' .
	...
	{
		$return['error_xch'] = 'Maintain at least ' . number_format($minimum_bal_usd, 5) .
			' ' . $settings_ancillaries->currency . '!';

		echo_json($return);
	}

	token_trade_sell($user_id, $amount, $total);
