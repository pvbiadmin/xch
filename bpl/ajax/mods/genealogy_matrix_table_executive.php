...
namespace BPL\Ajax\Mods\Matrix_Table\Genealogy\Executive;
...
function main($id_user, $plan)
{
	...
	details_matrix_table_executive(user($id_user), $plan);
...
function make_json($parent, $plan)
{
	...
	if (!empty($children))
	{
		foreach ($children as $child)
		{
			...
			details_matrix_table_executive($child, $plan);
 ...
 * @since version
 */
function details_matrix_table_executive($child, $plan): void
{
	echo '"id": "' . $child->id . '", ';
...
{
	$matrix_table_id = user_matrix_table_executive($user_id)->id;

	return fetch_all(
		'SELECT * ' .
		'FROM network_share_executive ' .
 ...
 * @since version
 */
function user_matrix_table_executive($user_id)
{
	return fetch(
		'SELECT * ' .
		'FROM network_share_executive ' .
