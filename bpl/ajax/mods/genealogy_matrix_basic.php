...
namespace BPL\Ajax\Mods\Matrix\Genealogy\Basic;
...
function main($id_user, $plan)
{
	...
	details_matrix_basic(user($id_user), $plan);
...
function make_json($parent, $plan)
{
	...
	if (!empty($children))
	{
		foreach ($children as $child)
		{
			...
			details_matrix_basic($child, $plan);
 ...
 * @since version
 */
function details_matrix_basic($child, $plan): void
{
	echo '"id": "' . $child->id . '", ';
...
{
	$matrix_id = user_matrix_basic($user_id)->id;

	return fetch_all(
		'SELECT * ' .
		'FROM network_matrix_basic a ' .
 ...
 * @since version
 */
function user_matrix_basic($user_id)
{
	return fetch(
		'SELECT * ' .
		'FROM network_matrix_basic ' .
